...
class SaliencyFilters:
    ...
    def __init__(self, uniqueness_sigma=0.25, distribution_sigma=20.0, k=6.0,
                 alpha=0.033, beta=0.033, n_segments=300, weight_g = 100, weight_s = 0.3):
        ...
        self.__weight_g = weight_g
        self.__weight_s = weight_s
    ###########################################################################
    # Public methods
    ###########################################################################

    def compute_saliency(self, image, array_of_gabor):
        """
        input:
        - image in RGB uint8
        output:
        ...
        superpixels, lab_color, rgb_color, position, gabor = self.__abstraction(image, array_of_gabor)
        ...
        # self.__weight_g = self.make_param(3, self.__weight_g)
        # self.__weight_s = self.make_param(3, self.__weight_s)

        result = torch.Tensor(result * 0.3 + gabor//100)
        return result

    ###########################################################################
    # Private methods
    ###########################################################################
    def make_param(self, in_channels, values, requires_grad=True, dtype=None):
        if dtype is None:
            dtype = 'float32'
    ...
    def __abstraction(self, image, array_of_gabor):
        nrgb = img_as_float(image)
        lab = rgb2lab(nrgb)
        nlab = (lab + np.array([0, 128, 128])) / np.array([100, 255, 255])
        narr = array_of_gabor.detach().numpy()
        narr = np.average(np.transpose(narr, (1,2,0)),axis=2)
        ...
        # mean_arr = np.zeros((n_segments,72))
        ...
        for superpixel in np.unique(superpixels):
            ...
            # mean_arr[superpixel, : ] = narr[mask, : ].mean(axis=0)

        return superpixels, mean_lab, mean_rgb, mean_position, narr
